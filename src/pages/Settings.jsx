  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">{[['excellent', 'A - Sangat Baik'], ['good', 'B - Baik'], ['fair', 'C - Cukup'], ['poor', 'D - Kurang'], ['heavyLateMinutes', 'Terlambat Berat (menit)']].map(([key, label]) => <label key={key} className="text-xs text-slate-500">{label}<input type="number" min="0" max={key === 'heavyLateMinutes' ? '480' : '100'} value={scoreForm[key] ?? ''} onChange={event => setScoreForm({ ...scoreForm, [key]: event.target.value })} className="mt-1 w-full p-2 rounded-xl border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white" /></label>)}</div>
